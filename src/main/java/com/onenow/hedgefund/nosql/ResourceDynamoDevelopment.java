...
@DynamoDBTable(tableName="CONTRACTS-DEVELOPMENT")
public class ResourceDynamoDevelopment extends ResourceDynamo {

    public ResourceDynamoDevelopment(){
        super();
    }

    public ResourceDynamoDevelopment(String lookup, String json) {
        super.lookup = lookup;
        super.json = json;
    }
    ...
    public static List<ContractResource> get(String lookup, DeployEnv nosqlEnv) {
        ...
        DynamoDBQueryExpression<ResourceDynamoDevelopment> queryExpression = getQuery(lookup);
        for(ResourceDynamoDevelopment item: Dynamo.mapper.query(ResourceDynamoDevelopment.class, queryExpression)) {
    ...
    private static DynamoDBQueryExpression<ResourceDynamoDevelopment> getQuery(String lookup) {
        ResourceDynamoDevelopment partitionKey = new ResourceDynamoDevelopment();
        partitionKey.setLookup(lookup);

        return new DynamoDBQueryExpression<ResourceDynamoDevelopment>()
